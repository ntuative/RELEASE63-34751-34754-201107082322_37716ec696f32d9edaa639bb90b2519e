package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_2061:uint = 1;
      
      protected static const const_2062:uint = 2;
      
      protected static const const_2059:uint = 4;
      
      protected static const const_2060:uint = 3;
      
      protected static const name_5:uint = 7;
      
      public static const const_1732:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1378:Boolean = false;
      
      public static const const_2030:int = 2;
      
      private static var var_1122:Array = new Array();
      
      private static var var_906:Array = new Array();
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         if(const_1378)
         {
            if(var_1122.indexOf(param1) == -1)
            {
               var_1122.push(param1);
            }
            method_1();
         }
         else
      ...
      protected static function method_1() : void
      {
         var _loc1_:* = null;
         var _loc2_:int = 0;
         if(const_1378)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_906[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_906.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_906.length < const_2030 && false)
            {
               _loc1_ = var_1122.shift() as LibraryLoader;
      ...
      {
         return !!this.var_1152 ? uint(this.var_1430 - this.var_2273) : uint(getTimer() - this.var_2273);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_432,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_44.close();
            }
      ...
      {
         if(!this.var_1152 && !_disposed && this.var_1429 > 0)
         {
            try
            {
               this.var_44.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_269,const_1732 - this.var_1429);
            if(this.var_3065)
            {
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_432 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_432 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_432,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_432,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_432))
               {
                  this.var_1430 = getTimer();
         ...
         if(this.var_514 == LibraryLoader.const_2060)
         {
         ...
         if(this.var_514 == LibraryLoader.name_5)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               method_1();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_432,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_715 = this.getDefinition(this._name) as Class;
         ...
         try
         {
            xmlClass = this.var_715.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_432,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         method_1();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_432,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
