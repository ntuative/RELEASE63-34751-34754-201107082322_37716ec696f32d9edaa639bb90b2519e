package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1696:String = "asset-libraries";
      
      private static const const_1700:String = "library";
      
      private static const const_1694:String = "service-libraries";
      
      private static const const_1693:String = "library";
      
      private static const const_1695:String = "component-libraries";
      
      private static const const_1699:String = "library";
      
      private static const const_1698:String = "error_data";
      
      private static const const_1701:String = "error_cat";
      
      private static const const_1697:String = "error_desc";
       
      
      private var var_417:LibraryLoaderQueue;
      
      private var var_904:IEventDispatcher;
      
      private var var_595:uint;
      
      private var var_418:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_427.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1259:
               debug("Core; using simple frame update handler");
               this.var_418 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_950:
               debug("Core; using complex frame update handler");
               this.var_418 = this.complexFrameUpdateHandler;
               break;
            case Core.const_782:
               debug("Core; using profiler frame update handler");
               this.var_418 = this.profilerFrameUpdateHandler;
               this.var_236 = new Profiler(this);
               attachComponent(this.var_236,[new IIDProfiler()]);
               var_427.addChild(new ProfilerViewer(this.var_236));
               break;
            case Core.const_674:
               debug("Core; using experimental frame update handler");
               this.var_418 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_418 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_595 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(var_427)
            {
               var_427.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_427 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1698,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1701,String(param3));
         ErrorReportStorage.setParameter(const_1697,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_904 = loadingEventDelegate;
         if(this.var_417 == null)
         {
            this.var_417 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1696)[0];
            if(node != null)
            {
               list = node.child(const_1700);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_211,e);
         }
         try
         {
            node = config.child(const_1694)[0];
            if(node != null)
            {
               list = node.child(const_1693);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_211,e);
         }
         try
         {
            node = config.child(const_1695)[0];
            if(node != null)
            {
               list = node.child(const_1699);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_211,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1361);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_904)
         {
            this.var_904.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_904)
         {
            ...
            this.var_904.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_904)
                  {
                     this.var_904.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_595 - this.var_417.length,this.var_595,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_236 ? 0 : this.var_1682 & 0;
         if(_loc3_ == Core.const_674)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_418(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_427.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_107)
         {
            t = getTimer();
            ...
            {
               if(this.var_735[priority] < priority)
               {
                  ++this.var_735[priority];
                  skip = true;
               }
            }
            if(!skip)
