package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager, IDisposable
   {
      ...
      private var var_187:AvatarRenderManager;
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_443.getValue(_loc4_) == null)
               {
                  this.var_443.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         if(param1 == null || this.var_521 == null)
         {
            return;
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_521.length > 0)
         {
            _loc1_ = this.var_521.pop() as IPetDataListener;
      ...
      {
         var _loc3_:IPetData = this.var_443.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      public function createPetFigureString(param1:int, param2:int, param3:uint) : String
      {
         ...
         var _loc4_:PetData = this.var_443.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_443.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_443.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.var_2667.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1453.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_619.getValue(_loc6_) == null)
               {
                  ...
                  if(param3 != null && this.var_521.indexOf(param3) == -1)
                  {
                     this.var_521.push(param3);
                  }
               }
            }
         }
