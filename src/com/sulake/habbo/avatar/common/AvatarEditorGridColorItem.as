package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridColorItem
   {
      ...
      private var var_1120:IPartColor;
      ...
      private function updateThumbData() : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc2_.bitmap = new BitmapData(this.var_1107.width,this.var_1107.height,true,0);
            _loc5_ = this.var_1107.clone();
            _loc5_.colorTransform(_loc5_.rect,this.var_1120.colorTransform);
         ...
         if(_loc3_ != null)
         {
            if(this._isSelected)
            {
               _loc3_.bitmap = new BitmapData(this.var_1106.width,this.var_1106.height,true,16777215);
               _loc3_.bitmap.copyPixels(this.var_1106,this.var_1106.rect,new Point(0,0),null,null,true);
               _loc3_.visible = true;
            }
            else
            {
         ...
         if(_loc4_)
         {
            if(this.var_1120)
            {
               switch(this.var_1120.clubLevel)
               {
                  case HabboClubLevelEnum.const_51:
                     _loc4_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_35:
                     _loc4_.style = HabboIconType.const_762;
                     _loc4_.visible = true;
                     break;
                  case HabboClubLevelEnum.const_41:
                     _loc4_.style = HabboIconType.const_901;
                     _loc4_.visible = true;
               }
            }
            else
