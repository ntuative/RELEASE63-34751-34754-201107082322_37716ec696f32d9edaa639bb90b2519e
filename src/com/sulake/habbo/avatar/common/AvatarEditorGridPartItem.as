package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_818:IFigurePartSet;
      ...
      private const const_2352:int = 13421772;
      
      private const const_2351:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_818 = param3;
         this._window = param1;
         this.var_1525 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1879 = Math.max(this.var_1879,_loc6_.colorLayerIndex);
            }
         }
         this.var_450 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_745 != null && !this.var_1881)
            {
               _loc3_ = this.var_745;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_818)
            {
               switch(this.var_818.clubLevel)
               {
                  case HabboClubLevelEnum.const_51:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_762;
                     break;
                  case HabboClubLevelEnum.const_41:
                     _loc2_.style = HabboIconType.const_901;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1525.color = this.const_2352;
         }
         else
         {
            this.var_1525.color = this.const_2351;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_450.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_450.isFigureReady(_loc1_))
         {
            this.var_450.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_387[_loc6_] + "_" + FigureData.const_144;
               _loc4_ = this.var_450.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_649 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1880)
               {
                  ...
                  this.var_1880 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_649.width,this.var_649.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_649.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_649.y;
               if(this.var_1881 && _loc2_.colorLayerIndex > 0)
               {
