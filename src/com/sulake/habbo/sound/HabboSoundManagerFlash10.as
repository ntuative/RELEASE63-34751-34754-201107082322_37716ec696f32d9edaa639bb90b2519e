package com.sulake.habbo.sound
{
   ...
   public class HabboSoundManagerFlash10 extends Component implements IHabboSoundManager, IUpdateReceiver
   {
      ...
      private static const const_1522:int = 60;
      
      private static const const_1521:int = 25165823;
      
      private static const const_1523:int = 16777215;
      ...
      private var var_1013:Number = 1.0;
      ...
      private var var_1534:ByteArray;
      ...
      public function HabboSoundManagerFlash10(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null, param4:Boolean = true)
      {
         ...
         events.addEventListener(TraxSongLoadEvent.const_799,this.onTraxLoadComplete);
      ...
      public function playSound(param1:String) : void
      {
         ...
         var _loc4_:* = null;
         var _loc5_:* = null;
         var _loc2_:HabboSound = this.var_1011.getValue(param1);
         if(_loc2_ == null)
         {
            _loc3_ = "";
            switch(param1)
            {
               case HabboSoundTypesEnum.const_1400:
                  _loc3_ = "sound_call_for_help";
                  break;
               case HabboSoundTypesEnum.const_1856:
               ...
               case HabboSoundTypesEnum.const_1306:
                  ...
                  break;
               case HabboSoundTypesEnum.const_1367:
                  _loc3_ = "sound_catalogue_cash";
                  break;
               case HabboSoundTypesEnum.const_1770:
            ...
            this.var_1011.add(param1,_loc2_);
         }
         _loc2_.volume = this.var_1013;
         _loc2_.play();
      }
      ...
      private function loadSample(param1:int) : void
      {
         ...
         _loc4_.load(_loc3_);
         this.var_654.add(_loc4_,param1);
      }
      ...
      private function addTraxSongForDownload(param1:int, param2:String) : IHabboSound
      {
         ...
         if(!_loc3_.ready)
         {
            this.var_1255.add(param1,_loc3_);
         }
         return _loc3_;
      }
      ...
      private function createTraxInstance(param1:int, param2:String, param3:Boolean = true) : TraxSequencer
      {
         ...
         var _loc5_:TraxSequencer = new TraxSequencer(param1,_loc4_,this.var_281,events);
         _loc5_.volume = this.var_1013;
      ...
      private function validateSampleAvailability(param1:TraxSequencer, param2:Boolean) : void
      {
         ...
         {
            if(this.var_281.getValue(int(_loc4_[_loc6_])) == null)
            {
               if(param2)
               {
      ...
      {
         if(this.var_128 == null || this._connection == null || this._roomEngine == null || this.var_254 != null)
         {
            return;
         }
         this.var_254 = new HabboMusicController(this,events,this._roomEngine.events,this._connection);
      }
      ...
      private function onTraxLoadComplete(param1:Event) : void
      {
         ...
         this.var_254.onSongLoaded(_loc2_.id);
      }
      
      private function ioErrorHandler(param1:Event) : void
      {
         events.dispatchEvent(new TraxSongLoadEvent(TraxSongLoadEvent.const_1761,this.var_828));
         this.var_828 = -1;
         this.var_827 = null;
      }
      ...
      private function storeVolumeSetting() : void
      {
         ...
         {
            this._connection.send(new SetSoundSettingsComposer(int(this.var_1013 * 100)));
         }
      }
      ...
      private function updateVolumeSetting(param1:Number) : void
      {
         ...
         this.var_254.updateVolume(this.var_1013);
      }
      ...
      private function processLoadedSample(param1:Sound) : void
      {
         ...
         {
            _loc2_ = this.var_654.remove(param1);
            if(this.var_281.getValue(_loc2_) == null)
            {
               this.var_1534.clear();
               _loc3_ = param1.length;
               param1.extract(this.var_1534,int(_loc3_ * 44.1));
               _loc4_ = new TraxSample(this.var_1534,_loc2_,TraxSample.const_2003,TraxSample.const_1998);
               this.var_281.add(_loc2_,_loc4_);
            }
         }
      }
      ...
      private function processLoadedSamples() : void
      {
         ...
         var _loc3_:* = null;
         var _loc4_:int = 0;
         if(this.var_1256.length > 0)
         {
            _loc1_ = getTimer();
            _loc2_ = _loc1_;
            while(_loc2_ - _loc1_ < const_1522 && this.var_1256.length > 0)
            {
               _loc3_ = this.var_1256.splice(0,1)[0];
               this.processLoadedSample(_loc3_);
               _loc2_ = getTimer();
            }
            if(this.var_654.length == 0 && this.var_828 != -1)
            {
               this.var_827.ready = true;
               _loc4_ = this.var_828;
               this.var_828 = -1;
               this.var_827 = null;
               events.dispatchEvent(new TraxSongLoadEvent(TraxSongLoadEvent.const_799,_loc4_));
               if(this.var_2554)
               {
      ...
      private function loadNextSong() : void
      {
         var _loc1_:int = 0;
         var _loc2_:* = null;
         if(this.var_827 == null && this.var_1255.length > 0)
         {
            _loc1_ = this.var_1255.getKey(0);
            _loc2_ = this.var_1255.remove(_loc1_);
            if(_loc2_ != null && !_loc2_.disposed)
            {
               this.validateSampleAvailability(_loc2_,true);
               if(_loc2_.ready)
               {
                  events.dispatchEvent(new TraxSongLoadEvent(TraxSongLoadEvent.const_799,_loc1_));
               }
               else
               {
                  this.var_827 = _loc2_;
      ...
      private function processSampleMemoryUsage() : void
      {
         ...
         var _loc1_:* = 0;
         var _loc2_:* = [];
         var _loc3_:Array = this.var_254.samplesIdsInUse;
         var _loc4_:int = 0;
         while(_loc4_ < this.var_281.length)
         {
            _loc5_ = this.var_281.getKey(_loc4_);
            _loc6_ = this.var_281.getWithIndex(_loc4_);
         ...
         if(_loc1_ > const_1521)
         {
            ...
            _loc8_ = 0;
            _loc9_ = 0;
            while(_loc8_ < _loc1_ - const_1523 && _loc9_ < _loc2_.length)
            {
            ...
            if(_loc7_.length > 0)
            {
               for each(_loc11_ in _loc7_)
               {
                  ...
                  _loc12_ = this.var_281.getValue(_loc11_) as TraxSample;
                  _loc12_.dispose();
                  this.var_281.remove(_loc11_);
               }
               this.var_254.samplesUnloaded(_loc7_);
            }
         }
      }
