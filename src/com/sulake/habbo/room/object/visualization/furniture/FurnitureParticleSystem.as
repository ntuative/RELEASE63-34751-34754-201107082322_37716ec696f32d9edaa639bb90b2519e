package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_280:IRoomObjectSprite;
      ...
      private var var_1250:Number = 1.0;
      
      private var var_653:BitmapData;
      
      private var var_825:ColorTransform;
      
      private var var_1531:ColorTransform;
      
      private var var_1896:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_825 = new ColorTransform();
         this.var_825.alphaMultiplier = 1;
         this.var_1531 = new ColorTransform();
         this.var_1896 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_113 = this.var_1249[param1];
         this.var_1005 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1895 >= 0)
         {
            this.var_280 = this._visualization.getSprite(this.var_1895);
            if(this.var_280 && this.var_280.asset)
            {
               if(this.var_280.width <= 1 || this.var_280.height <= 1)
               {
                  return;
               }
               if(this.var_99 == null)
               {
                  this.var_99 = this.var_280.asset.clone();
                  if(this.var_825.alphaMultiplier != 1)
                  {
                     this.var_653 = new BitmapData(this.var_99.width,this.var_99.height,true,4278190080);
                  }
               }
               this.var_2275 = -this.var_280.offsetX;
               this.var_2274 = -this.var_280.offsetY;
               this.var_280.asset = this.var_99;
            }
            if(this.var_99)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_113.y * this.var_1250;
         }
         return 0;
      }
      ...
      {
         if(!this.var_113 || !this.var_280)
         {
            return;
         }
         if(this.var_99 && this.var_280.asset != this.var_99)
         {
            this.var_280.asset = this.var_99;
         }
         if(this.var_1005)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1005 && this.var_113.hasIgnited)
         {
            this.var_1005 = true;
         }
         _loc8_ = this.var_1178 * this.var_1250;
         this.var_113.update();
         if(this.var_1005)
         {
            if(this.var_113.roomObjectSpriteId >= 0)
            ...
            {
               this.var_99.draw(this.var_653,this.var_1896,this.var_825,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_113.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2275 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1250;
               _loc4_ = this.var_2274 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1250;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1531.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_99.draw(_loc7_,this._translationMatrix,this.var_1531,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2552 = parseInt(param1.@size);
         this.var_1895 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1178 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1250 = this.var_2552 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_825.alphaMultiplier = this._blend;
