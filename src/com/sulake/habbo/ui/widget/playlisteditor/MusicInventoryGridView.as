package com.sulake.habbo.ui.widget.playlisteditor
{
   ...
   public class MusicInventoryGridView
   {
      ...
      private var var_732:IItemGridWindow;
      
      private var _items:Map;
      
      private var _widget:PlayListEditorWidget;
      
      private var var_117:MusicInventoryGridItem;
      ...
      public function destroy() : void
      {
         ...
         {
            this.var_732.destroyGridItems();
            this.var_732 = null;
         }
         if(this.var_254 != null)
         {
            if(this.var_254.events != null)
            {
      ...
      public function refresh() : void
      {
         ...
         this.var_732.removeGridItems();
         var _loc1_:Map = this._items;
         var _loc2_:Map = new Map();
         var _loc3_:Array = _loc1_.getKeys();
         this._items = new Map();
         var _loc4_:int = this.var_254.getSongDiskInventorySize();
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            _loc7_ = this.var_254.getSongDiskInventoryDiskId(_loc5_);
            _loc8_ = this.var_254.getSongDiskInventorySongId(_loc5_);
            _loc9_ = this.var_254.getSongInfo(_loc8_);
            _loc10_ = null;
            _loc11_ = null;
            if(_loc9_ != null)
            {
            ...
            this.var_732.addGridItem(_loc12_.window);
      ...
      public function method_13() : void
      {
         ...
         {
            this.var_117.playButtonState = MusicInventoryGridItem.const_1337;
         }
      }
      ...
      public function setPreviewIconToPlay() : void
      {
         if(this.var_117 != null)
         {
            this.var_117.playButtonState = MusicInventoryGridItem.const_669;
         }
      }
      ...
      private function gridItemEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         var _loc4_:int = 0;
         var _loc5_:* = null;
         var _loc3_:* = param1.type == WindowMouseEvent.const_166;
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK || _loc3_)
         {
            if(param2.name == "button_to_playlist" || _loc3_)
            {
               if(this.var_117 != null)
               {
                  this.var_117.deselect();
                  this.stopPreview();
                  this._widget.sendAddToPlayListMessage(this.var_117.diskId);
                  this.var_117 = null;
               }
            }
            else if(param2.name == "button_play_pause")
            {
               if(this.var_117.playButtonState == MusicInventoryGridItem.const_669)
               {
                  this.var_117.playButtonState = MusicInventoryGridItem.const_1333;
                  this._widget.playUserSong(this.var_117.songId);
               }
               else
               {
                  this.stopPreview();
            ...
            {
               _loc4_ = this.var_732.getGridItemIndex(param1.window);
               if(_loc4_ != -1)
               {
                  _loc5_ = this._items.getWithIndex(_loc4_);
                  if(_loc5_ != this.var_117)
                  {
                     if(this.var_117 != null)
                     {
                        this.var_117.deselect();
